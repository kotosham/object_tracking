...
SYSTEM_PROMPT = (
    ...
    "positive distance_m. "
    "DETECT_ALL -- refresh fixed-vocabulary scene context for the current target "
    "(for office targets, this means office furniture/context classes). It is a "
    "perception-only action, not a movement action, and it does not approach "
    "context objects. "
    ...
    "If neither target nor useful context is visible, use blind_scan with a "
    "meaningful TURN or a cautious DRIVE_FORWARD through mapped free space; use "
    "DETECT_ALL only when you need to refresh the fixed context vocabulary. Start "
    "the rationale with 'blind_scan:'. "
