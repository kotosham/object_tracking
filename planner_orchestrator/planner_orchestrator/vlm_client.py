...
SYSTEM_PROMPT = (
    ...
    "semantic relevance); and your running notes. Use known distance_m values "
    ...
    "office chair), use semantic_explore: choose TURN toward that side, or DRIVE_FORWARD "
    "if the useful context is centered and the map looks free. Start the rationale with "
    "'semantic_explore:'. Some target-like context detections may also appear in "
    "visible_marks; those have been promoted to real target candidates, so you may "
    "DRIVE_TO_VISIBLE them. If a target-like object appears only in context_marks, "
